...
def random_split_dataframe(df : pd.DataFrame, val_size : float = 0.15, test_size : float = 0.15):
    ...
    train_idx, test_idx, *_ = train_test_split(df_idx, test_size=test_size)

    train_idx, val_idx, *_ = train_test_split(train_idx, test_size=val_size/train_size)
...
if "__main__" == __name__:
    ...
    simple_passes = non_overlap_df[non_overlap_df.class_coarse == "Pass"]
    ...
    train_idx, val_idx, test_idx = random_split_dataframe(full_df)
    train_df = full_df.iloc[train_idx]
    train_df.reset_index(inplace=True, drop=True)
    print(train_df.value_counts(["class_coarse"]))
    train_df.to_json(data_path / f"meta3d_{sql}_{sr}_{'overlap' if overlap else 'nooverlap'}_train_balanced.jsonl", lines=True, orient="records")

    val_df = full_df.iloc[val_idx]
    val_df.reset_index(inplace=True, drop=True)
    print(val_df.value_counts(["class_coarse"]))
    val_df.to_json(data_path / f"meta3d_{sql}_{sr}_{'overlap' if overlap else 'nooverlap'}_val_balanced.jsonl", lines=True, orient="records")


    test_df = full_df.iloc[test_idx]
    test_df.reset_index(inplace=True, drop=True)
    print(test_df.value_counts(["class_coarse"]))
    test_df.to_json(data_path / f"meta3d_{sql}_{sr}_{'overlap' if overlap else 'nooverlap'}_test_balanced.jsonl", lines=True, orient="records")
